        ...
        <thead>
          <tr className="border-b border-slate-200/60 dark:border-slate-700/60">
            {columns.map((col, idx) => (
              <th
                key={idx}
                className={`py-4 px-6 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-xs whitespace-nowrap bg-slate-50/50 dark:bg-slate-800/50 first:rounded-tl-lg last:rounded-tr-lg ${col.className || ""}`}
              >
                {col.header}
              </th>
            ))}
          ...
          {data.length === 0 ? (
            ...
            data.map((row, rowIndex) => (
              <tr
                ...
                className="hover:bg-indigo-50/30 dark:hover:bg-slate-800/80 transition-colors duration-200 group text-slate-800 dark:text-slate-200"
              >
