...
function StatCard({ label, value, sub, icon, gradient }) {
  return (
    <Card className={`relative overflow-hidden group border border-slate-200 dark:border-slate-700`}>
      <div className="absolute top-0 right-0 p-4 opacity-10 text-indigo-500 transform group-hover:scale-110 transition-transform duration-500">
        {icon}
      </div>
      <div className="relative z-10 flex flex-col items-start h-full justify-between gap-4">
        <div className="w-12 h-12 rounded-xl bg-indigo-100 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-400 flex items-center justify-center mb-2">
          {icon}
        </div>
        <div>
          <h4 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight mb-1">{value}</h4>
